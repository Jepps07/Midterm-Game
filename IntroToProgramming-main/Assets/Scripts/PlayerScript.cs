...

{
    ...
    public float TimeLeft = 30f; // 30-second timer
    ...
    private void OnCollisionEnter2D(Collision2D other)
    {
        ...
        if (coin != null)
        {
            ...
            if (TimeLeft <= 0)
            {
                TimeLeft = 0;
                Debug.Log("Time's up!");
                Die(); // Optional: End the game when the timer runs out
            }
            else
            {
                Debug.Log("Time left: " + Mathf.Ceil(TimeLeft));
